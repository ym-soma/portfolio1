$(function(){
  ...
  function slideTimer(){
    // 現在の画像をフェードアウト
    $(".slide_img li:nth-child(" + current + ")").fadeOut(duration);

    // 次の画像をフェードイン
    $(".slide_img li:nth-child(" + next + ")").fadeIn(duration);
    ...
    // targetクラスを削除
    $(".slide_btn li a").removeClass("target");

    // 現在のボタンにtargetクラスを追加
    $(".slide_btn li:nth-child(" + current + ") a").addClass("target");
  }
